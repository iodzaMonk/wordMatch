...
{/if}
<div class="text-white flex flex-col gap-5 items-center">
<div class="h-2 w-4/5 bg-white/10 rounded-full overflow-hidden">
    ...
    <div class="text-7xl">{target.character}</div>
    ...
    <div class="flex gap-10 text-2xl">
        {#each shuffled as option (option.character)}
            ...
            class:correct-style={selected === option && option.character === target.character}
            class:wrong-style={selected=== option && option.character !== target.character}
            >{option.definition}</button>
        {/each}
    </div>
</div>